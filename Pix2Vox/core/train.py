...
from utils.collate_fns import DINOCollator
...
from models.projectors import DINOProjector
from models.dinov2 import DINOv2WithRegistersForInference
...
def train_net(cfg):
    ...
    refiner = Refiner(cfg)
    merger = Merger(cfg)
    dino_projector = DINOProjector()
    dino = DINOv2WithRegistersForInference()
    dino_collator = DINOCollator(dino.processor)
    ...
    print('[DEBUG] %s Parameters in DINO Projector: %d.' % (dt.now(), utils.network_utils.count_parameters(dino_projector)))
    ...
    dino_projector.apply(utils.network_utils.init_weights)
    ...
    if cfg.TRAIN.POLICY == 'adam':
        ...
        dino_projector_solver = torch.optim.Adam(dino_projector.parameters(), lr=cfg.TRAIN.DINO_PROJECTOR_LEARNING_RATE,
                                                 betas=cfg.TRAIN.BETAS)
        ...
        merger_solver = torch.optim.SGD(merger.parameters(),
                                        ...
                                        momentum=cfg.TRAIN.MOMENTUM)
        dino_projector_solver = torch.optim.SGD(dino_projector.parameters(), lr=cfg.TRAIN.DINO_PROJECTOR_LEARNING_RATE,
                                                momentum=cfg.TRAIN.MOMENTUM)
    else:
    ...
    merger_lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(merger_solver,
                                                               ...
                                                               gamma=cfg.TRAIN.GAMMA)
    dino_projector_lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(dino_projector_solver,
                                                                       milestones=cfg.TRAIN.DINO_PROJECTOR_LR_MILESTONES,
                                                                       gamma=cfg.TRAIN.GAMMA)
    ...
    if torch.cuda.is_available():
        ...
        dino_projector = torch.nn.DataParallel(dino_projector).cuda()
    ...
    if 'WEIGHTS' in cfg.CONST and cfg.TRAIN.RESUME_TRAIN:
        ...
        # dino_projector.load_state_dict(checkpoint['dino_projector_state_dict'])
    ...
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHES):
        ...
        merger.train()
        refiner.train()
        dino_projector.train()

        batch_end_time = time()
        n_batches = len(train_data_loader)
        for batch_idx, (taxonomy_names, sample_names, rendering_images,
                        ground_truth_volumes, img_paths) in enumerate(train_data_loader):
            # Measure data time
            ...
            # DINO
            flat_imgs_for_dino, batch_size, num_img_per_item = dino_collator.collate_for_dino(img_paths)
            dino_encoded_feats = dino.embed(flat_imgs_for_dino)
            B_times_N, dino_embed_dim, dino_embed_h, dino_embed_w = dino_encoded_feats.shape
            dino_encoded_feats = dino_encoded_feats.reshape(batch_size, num_img_per_item, dino_embed_dim, dino_embed_h,
                                                            dino_embed_w)
            dino_projected_feats = dino_projector(dino_encoded_feats)

            # Train the encoder, decoder, refiner, and merger
            image_features = encoder(rendering_images)

            # Merge Encoder and DINO features
            image_features = image_features + dino_projected_feats
        ...
        dino_projector_lr_scheduler.step()
        ...
        if (epoch_idx + 1) % cfg.TRAIN.SAVE_FREQ == 0:
            ...
            utils.network_utils.save_checkpoints(cfg, os.path.join(ckpt_dir, 'ckpt-epoch-%04d.pth' % (epoch_idx + 1)),
                                                 ...
                                                 refiner, refiner_solver, merger, merger_solver, dino_projector,
                                                 dino_projector_solver, best_iou, best_epoch)
        if iou > best_iou:
            if not os.path.exists(ckpt_dir):
                os.makedirs(ckpt_dir)
            ...
            utils.network_utils.save_checkpoints(cfg, os.path.join(ckpt_dir, 'best-ckpt.pth'), epoch_idx + 1, encoder,
                                                 ...
                                                 merger, merger_solver, dino_projector, dino_projector_solver,
                                                 best_iou, best_epoch)
